...
- 更新UI组件的现代化改进：DashboardGrid和WidgetFrame的样式优化，以及整体布局的紧凑化设计
- 新增圆角系统和玻璃材质效果的详细说明
- 更新WidgetFrame的组件包装机制，包含新的圆角和玻璃效果
- 更新DashboardGrid的网格系统参数，实现更紧凑的布局设计
- 新增噪声纹理和光学边缘效果的说明
...
- 视图层：DashboardGrid 负责布局聚合与渲染；GridLayer 封装 react-grid-layout；WidgetFrame 提供统一的组件容器与现代化视觉样式。
...
- [WidgetFrame.tsx:1-9](file://src/components/layout/WidgetFrame.tsx#L1-L9)
...
- [tokens.css:1-263](file://src/styles/tokens.css#L1-L263)
...
- DashboardGrid：负责根据启用的组件生成可见布局，计算 xs 断点布局，处理布局变更回调，控制编辑态与移动端行为，并将子组件包裹在 WidgetFrame 中。**更新** 使用更紧凑的布局参数，实现现代化的视觉效果。
- GridLayer：基于 react-grid-layout 的响应式网格容器，配置断点、列数、行列高、边距、拖拽与缩放能力，并通过 draggableHandle 指定拖拽句柄。**更新** 采用更紧凑的网格间距和行高设计。
- WidgetFrame：为每个小部件提供统一的背景、圆角、阴影、玻璃效果与编辑态高亮边框。**更新** 集成了新的圆角系统 `rounded-card` 和玻璃材质效果，包含噪声纹理和光学边缘。
- ShortcutsGrid：使用容器查询实现响应式快捷方式网格布局，支持多种断点下的列数动态调整，断点阈值已提升至 1120px 和 1350px。
...
- tokens.css：设计令牌系统，定义圆角半径、阴影、颜色等视觉变量。**更新** 包含新的圆角系统和玻璃材质参数。
- cn.ts：类名合并工具，使用 clsx 和 tailwind-merge 优化类名处理。
...
布局系统采用"视图层 + 状态层 + 设置与交互层"的分层设计，结合 react-grid-layout 实现响应式网格与拖拽交互，使用 zustand 管理状态并通过 Chrome Storage 实现多页面同步。**更新** 现代化的UI组件设计，包括圆角系统、玻璃材质效果和噪声纹理，为用户提供更精致的视觉体验。
...
participant WF as "WidgetFrame.tsx"
...
- **更新** 紧凑化设计：使用更紧凑的布局参数，包括更小的边距和行高，实现现代化的视觉效果。
...
### GridLayer：react-grid-layout 集成与现代化拖拽交互

- 断点与列数：lg/md/sm/xs 分别配置不同列数与断点阈值，xs 使用 xsLayout 单列。
- **更新** 紧凑化参数：rowHeight 设置为 60px，margin 为 [16, 16]，实现更紧密的网格布局。
...
### WidgetFrame：现代化组件包装机制

- **更新** 圆角系统：使用 `rounded-card` 类实现统一的圆角半径，支持不同的材质风格。
- **更新** 玻璃材质效果：根据 transparent 选择透明或带背景、圆角、阴影与玻璃纹理。
- **更新** 噪声纹理：通过 `glass-noise` 类添加噪声纹理，增强玻璃材质的真实感。
- **更新** 光学边缘效果：使用 `backdrop-blur-glass` 实现模糊玻璃效果，支持不同的模糊强度。
...
+backdrop-blur-glass : var(--blur-widget)
+glass-noise : noise-texture
+bg-surface : var(--surface)
+shadow-card : var(--shadow-card)
}
WidgetFrame --> ModernStyles : "应用"
```
...
### ShortcutsGrid：容器查询响应式布局

- 容器查询网格：使用 .shortcut-grid 类实现响应式网格布局。
- 动态列数调整：根据容器宽度自动调整列数，从 4 列到 12 列逐步增加。
- 断点配置：支持 320px、480px、1120px、1350px 四个断点，列数从 6 列递增到 12 列。10 列触发条件从 640px 提升至 1120px，12 列触发条件从 960px 提升至 1350px。
...
- 容器查询断点：.shortcut-grid 支持 320px、480px、1120px、1350px 四个断点，列数从 6 列递增到 12 列。10 列触发条件从 640px 提升至 1120px，12 列触发条件从 960px 提升至 1350px。
- **更新** 圆角系统：统一的 `--radius-card` 变量，支持不同的材质风格（基础 20px，玻璃模式 26px）。
- **更新** 玻璃材质：通过 `backdrop-blur-glass` 和 `glass-noise` 类实现现代化的玻璃效果。
- **更新** 光学边缘：使用 `inset` 阴影实现真实的光学边缘效果。
...
- 组件耦合：DashboardGrid 依赖 GridLayer 与 WidgetFrame；GridLayer 依赖 react-grid-layout；WidgetFrame 依赖 cn 工具与 CSS；ShortcutsGrid 依赖容器查询样式。
- 状态耦合：DashboardGrid 依赖 useLayoutStore 与 useSettingsStore；LayoutSection 依赖 useLayoutStore 与其他 store。
- 外部依赖：react-grid-layout、zustand、zustand/persist、Chrome Storage API、Tailwind CSS 容器查询支持。
- **更新** 设计系统：WidgetFrame 依赖 tokens.css 中的设计令牌，实现统一的视觉语言。
- 循环依赖：未发现循环依赖迹象。
...
- **更新** 圆角系统优化：通过 CSS 变量统一管理圆角半径，减少重复计算。
- **更新** 玻璃材质性能：使用硬件加速的 backdrop-filter，确保流畅的视觉效果。
...
- **更新** 圆角效果异常
  - 检查 `rounded-card` 类是否正确应用到组件上。
  - 确认 CSS 变量 `--radius-card` 在 tokens.css 中正确定义。
  - 参考路径：[WidgetFrame.tsx:15](file://src/components/layout/WidgetFrame.tsx#L15)，[tokens.css:3](file://src/styles/tokens.css#L3)
- **更新** 玻璃材质效果问题
  - 检查 `backdrop-blur-glass` 和 `glass-noise` 类是否正确应用。
  - 确认 CSS 变量 `--blur-widget` 和 `--noise-texture` 在 tokens.css 中正确定义。
  - 参考路径：[WidgetFrame.tsx:16](file://src/components/layout/WidgetFrame.tsx#L16)，[tokens.css:12](file://src/styles/tokens.css#L12)
- **更新** 容器查询布局异常
  - 检查 .shortcut-grid 类是否正确应用到容器元素。
  - 确认浏览器支持容器查询特性（现代浏览器已支持）。
  - 验证断点值是否符合新的阈值：320px、480px、1120px、1350px。
  - 如果在 1120px-1350px 范围内没有看到 10 列布局，在 1350px 以上没有看到 12 列布局，请检查断点阈值配置。
...
- [WidgetFrame.tsx:15-16](file://src/components/layout/WidgetFrame.tsx#L15-L16)
- [tokens.css:3](file://src/styles/tokens.css#L3)
- [tokens.css:12](file://src/styles/tokens.css#L12)
...
**更新的现代化UI组件设计**为布局系统带来了显著的视觉改进：

- 圆角系统：统一的 `rounded-card` 类，支持 20px（基础）和 26px（玻璃模式）的圆角半径
- 玻璃材质效果：通过 `backdrop-blur-glass` 和 `glass-noise` 类实现逼真的玻璃质感
- 噪声纹理：添加微妙的噪声纹理增强材质的真实感
- 光学边缘：使用 inset 阴影模拟真实的光学边缘效果
- 紧凑化设计：更紧密的网格间距和行高，提供现代化的视觉体验

这些改进使得布局系统不仅功能强大，而且在视觉上也达到了现代Web应用的标准。容器查询响应式布局功能进一步增强了系统的适应性，断点阈值已提升至 1120px 和 1350px，更好地适配现代显示器分辨率，特别是标准笔记本电脑分辨率（约 1366px）。移除 XL 特定的布局约束类使布局在标准分辨率下更加稳定。
...
- **更新** 圆角系统参考
  - `rounded-card`：统一的圆角类名
  - `--radius-card`：CSS 变量，基础值 20px，玻璃模式 26px
  - `--radius-btn`：按钮圆角，10px
- **更新** 玻璃材质参考
  - `backdrop-blur-glass`：模糊玻璃效果
  - `glass-noise`：噪声纹理叠加
  - `--blur-widget`：基础模糊强度 40px，玻璃模式 100px
  - `--surface`：表面颜色，支持透明度
- **更新** 容器查询断点参考
...
- 扩展建议
  ...
  - **更新** 圆角系统扩展：基于 `--radius-card` 变量创建更多圆角变体
  - **更新** 玻璃材质扩展：利用 `backdrop-blur-glass` 和 `glass-noise` 创建更多材质效果
