...
- 壁纸选择器从复杂组件重构为useWallpaperPicker hook，提升代码复用性和测试覆盖率
- 新增完整的hook测试套件，包含275行测试代码
- 保持原有UI组件结构，但使用hook提供业务逻辑
- 优化了收藏功能的状态管理逻辑
...
**更新** 本版本重点介绍了从复杂组件向hook架构的重构，提升了代码的可测试性和可维护性。
...
- Hook层：useWallpaperPicker（壁纸选择逻辑）
...
- [src/components/settings/WallpaperPicker.tsx:1-144](file://src/components/settings/WallpaperPicker.tsx#L1-L144)
...
- [src/store/useFavoriteWallpapersStore.ts:1-48](file://src/store/useFavoriteWallpapersStore.ts#L1-L48)
...
- [src/lib/useWallpaper.ts:1-109](file://src/lib/useWallpaper.ts#L1-L109)
...
- 壁纸选择器Hook（useWallpaperPicker）：提供预设、收藏、上传、随机墙纸、收藏当前、暗化调节等核心逻辑，支持完整的单元测试。
- UI组件（WallpaperPicker）：基于useWallpaperPicker提供的逻辑构建用户界面，保持原有交互体验。
...
**更新** 核心变化是从复杂的WallpaperPicker组件重构为useWallpaperPicker hook，将业务逻辑与UI分离，提升了代码的可测试性和可维护性。
...
壁纸配置模块遵循"前台 UI + Hook逻辑层 + 后台服务 + 持久化存储 + 缓存与网络"的分层架构。前台负责交互与渲染，Hook层提供业务逻辑，后台服务处理第三方 API 请求，存储层保证跨会话一致性，缓存层降低网络与解码成本。

```mermaid
sequenceDiagram
participant U as "用户"
participant WP as "WallpaperPicker"
participant UP as "useWallpaperPicker"
...
U->>WP : "点击随机排行榜"
WP->>UP : "调用handleRandom()"
UP->>WH : "fetchRandomWallhaven(策略)"
WH->>BG : "chrome.runtime.sendMessage(type : 'fetch-wallhaven')"
BG-->>WH : "返回壁纸结果(id/url/thumb/resolution)"
WH-->>UP : "WallhavenResult"
UP->>S : "setWallpaper(url)"
UP->>F : "可选：addFavorite(收藏当前)"
UP-->>WP : "返回更新后的状态"
WP-->>U : "触发 useWallpaper 渲染新壁纸"
```

**图表来源**

- [src/components/settings/WallpaperPicker.tsx:84-87](file://src/components/settings/WallpaperPicker.tsx#L84-L87)
- [src/components/settings/useWallpaperPicker.ts:64-87](file://src/components/settings/useWallpaperPicker.ts#L64-L87)
...
- 测试覆盖
  - 完整的单元测试套件，覆盖初始化状态、随机获取、收藏功能、错误处理等场景。

**更新** useWallpaperPicker是一个独立的React Hook，提供所有壁纸选择相关的业务逻辑，支持完整的单元测试，显著提升了代码质量和可维护性。
...
### UI组件（WallpaperPicker）

- 功能要点
  - 基于useWallpaperPicker提供的逻辑构建用户界面。
  - 保持原有的预设壁纸网格、收藏区域、随机排行榜、上传功能等交互。
  - 实时响应hook返回的状态变化。
- 关键交互
  - 通过useWallpaperPicker返回的函数和状态控制UI行为。
  - 支持策略切换、暗化强度调节、收藏操作等。

**更新** WallpaperPicker现在是一个轻量级的UI组件，完全依赖useWallpaperPicker hook提供业务逻辑，实现了关注点分离。

**章节来源**

- [src/components/settings/WallpaperPicker.tsx:1-144](file://src/components/settings/WallpaperPicker.tsx#L1-L144)
- [src/components/settings/useWallpaperPicker.ts:102-117](file://src/components/settings/useWallpaperPicker.ts#L102-L117)
...
- [src/lib/useWallpaper.ts:1-109](file://src/lib/useWallpaper.ts#L1-L109)
- [src/lib/wallpaperCache.ts:1-94](file://src/lib/wallpaperCache.ts#L1-L94)
- [src/components/settings/WallpaperPicker.tsx:112-118](file://src/components/settings/WallpaperPicker.tsx#L112-L118)
...
- 组件耦合
  - WallpaperPicker 依赖 useWallpaperPicker hook 和 WallpaperFavorites 组件。
  - useWallpaperPicker 依赖设置存储、收藏存储、Wallhaven 客户端与 UI 工具。
...
**更新** 依赖关系发生了重要变化：WallpaperPicker现在只依赖useWallpaperPicker hook，实现了UI与逻辑的分离。

```mermaid
graph LR
WP["WallpaperPicker.tsx"] --> UP["useWallpaperPicker.ts"]
UP --> S1["useSettingsStore.ts"]
UP --> S2["useFavoriteWallpapersStore.ts"]
UP --> WH["wallhaven.ts"]
...
- Hook优化：useWallpaperPicker 使用useMemo优化收藏状态计算，避免不必要的重新渲染。

**更新** 新增了useWallpaperPicker hook的性能优化，包括useMemo的使用来避免不必要的重新计算。
...
- [src/lib/wallpaperTint.ts:1-163](file://src/lib/wallpaperTint.ts#L1-L163)
- [src/lib/theme.ts:87-95](file://src/lib/theme.ts#L87-L95)
- [src/components/settings/useWallpaperPicker.ts:41-44](file://src/components/settings/useWallpaperPicker.ts#L41-L44)
...
- 主题色调未更新
  ...
  - 检查 CSS 变量是否被正确设置。
- 收藏无法同步
  - 础确认 chrome.storage.onChanged 是否注册成功。
  - 检查 registerRemoteSync 的回调是否执行。
- Hook逻辑异常
  - 检查useWallpaperPicker的测试用例是否通过。
  - 确认状态更新逻辑是否正确处理lastResult和current的匹配。

**更新** 新增了useWallpaperPicker hook相关的故障排除指导。
...
- [src/components/settings/useWallpaperPicker.test.ts:138-151](file://src/components/settings/useWallpaperPicker.test.ts#L138-L151)

## 结论

壁纸配置模块以清晰的分层设计实现了"所见即所得"的壁纸体验：前台提供直观的交互入口，Hook层封装了复杂的业务逻辑，后台服务保障第三方资源的稳定获取，存储与缓存兼顾一致性与性能，主题与色调系统进一步提升了可读性与美观度。通过可扩展的源接入与完善的错误处理，该模块具备良好的可维护性与可扩展性。

**更新** 本次重构将复杂的组件逻辑抽象为独立的useWallpaperPicker hook，显著提升了代码的可测试性、可维护性和可复用性，同时保持了原有的用户体验和功能完整性。
...
**更新** 新增了useWallpaperPicker hook的测试指南，展示了完整的测试覆盖范围和测试场景。
